...
def predict(args_):
    ...
    print('Loading weights from {}'.format('./model/kitti_frcnn_last.hdf5'))
    ...
    if os.path.isdir(path):
        ...
        for idx, img_name in enumerate(sorted(os.listdir(path))):
            ...
            label=0 if '_'in img_name else 1
            labels.append(label)
            pred=predict_single_image(os.path.join(path, img_name), model_rpn,
                                 model_classifier_only, cfg, class_mapping)
            preds.append(pred)
        np.save('label.npy',np.asarray(labels))
        np.save('pred.npy',np.asarray(preds))
